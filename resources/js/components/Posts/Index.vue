...
<template>
    ...
    <div class="p-6 bg-gray-50">
        <!-- Search Section -->
        <div class="mb-6">
            <input v-model="search_global" type="text" placeholder="Search for posts..."
                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" />
        </div>

        <!-- posts Grid -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="post in posts.data" class="bg-white shadow-md rounded-lg overflow-hidden">
                <div class="p-4">
                    <img :src="post.image" class="w-full h-48 object-cover" />
                    <h2 class="text-lg font-bold text-gray-800">{{ post.title }}</h2>
                    <p class="text-sm text-gray-600 mb-2">
                        <span class="font-medium">Category:</span> {{ post.category }}
                    </p>
                    <p class="text-sm text-gray-600 truncate">
                        {{ post.content }}
                    </p>
                    <p class="text-xs text-gray-500 mt-4">Published: {{ post.created_at }}</p>
                    <div class="mt-4 flex justify-between">


                        <a href="#" @click.prevent="likePost(post.id)" class="ml-2">
                            <span v-if="post.users">
                            ...
                            <!-- <span v-if="post.users">❤️</span>
                            <span v-else>🤍</span> -->
                        </a>

                        <span class="text-sm font-medium text-gray-700">{{ post.likes_count }}</span>
                    </div>
                </div>
            </div>
        </div>
...
const { posts, getPosts, deletePost, likePost } = usePosts()
